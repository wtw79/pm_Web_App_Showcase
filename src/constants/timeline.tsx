export const timeline = [
  {
    ...
    responsibilities: [
      "文件管理顶级 Tab：逻辑视图（按标的聚合完整分组）+ 类型视图下拉筛选（全部媒体（含 PDF）/全部图片/全部 PDF/报表/截图）",
      "分组导入可设为标的字段：欢迎页 PDF→welcomePdf、欢迎页图片→welcomeImages、竞价页图片→media（标的图片）；媒体映射修正（标的图片落竞价页目录，与欢迎页图片语义独立）",
      "系统内文件操作：图片/PDF 内嵌预览（官方 pdf.js viewer）、下载导出、多选 zip 打包、删除进系统回收站（媒体列表同步移除）、外部变更自动同步（fs.watch）",
      "归档相对路径显示（不暴露本机绝对路径）+ 文件重命名 + 右键菜单 + 系统资源管理器入口",
      "删除标的物理文件彻底清理（3 根级目录 + 回收站 + 校验无残留）",
      "标的详情单开一页：独立路由浏览详情，返回一键回标的管理",
      "投屏性能：持久化异步化（push-state 防抖落盘）+ PDF 翻页/缩放即时通道 + 报价 30fps 实时通道",
      "投屏 PDF 官方 viewer 迁移：PDFSinglePageViewer 组件、自算 fit 缩放基准修正、滚动式翻页兼容",
      "软键盘与报价体验：键盘设置只生成数字键、报价键盘固定默认布局、点击外部稳定收起",
      "投屏预览帧流重写：预览窗实时跟随副屏画面（帧级延迟、任意尺寸清晰、对齐桌面版 DWM 缩略图语义）",
      "系统测试体系升级 L1-L4：IPC 接口层自动化测试（通道直驱 + 落盘回读校验）+ 性能基准，逻辑 / 接口 / 性能分层",
      "报错规范体系：统一错误码（A-BB-CCC）+ traceId 全链路审计，失败可回溯",
      "成交归档截图可靠性：轮询「已成交」弹窗渲染完成后再截图，杜绝截到旧画面",
    ],
  },
  {
