export const timeline = [
  {
    ...
    responsibilities: [
      "文件管理顶级 Tab：逻辑视图（按标的聚合：欢迎页媒体/竞价页图片/成交归档）+ 物理视图 + 类型视图（下拉切换：全部媒体/报表/截图）",
      "系统内文件操作：图片/PDF 内嵌预览、下载导出、多选 zip 打包、删除进系统回收站（媒体列表同步移除）",
      "逻辑视图「仅媒体/报表/截图」过滤：默认只渲染归档核心文件，界面聚焦",
      "物理视图整理：新建文件夹/重命名/剪贴板复制剪切粘贴（移动/复制）/排序 + 文件/目录/空白区差异化右键菜单",
      "系统资源管理器入口：在文件管理内直接打开/定位归档目录",
      "标的详情单开一页：独立路由浏览详情，返回一键回标的管理",
      "投屏 PDF 渲染兼容修复：pdfjs 降级至自带 toHex 兼容的稳定版，旧内核宿主也能正常渲染",
