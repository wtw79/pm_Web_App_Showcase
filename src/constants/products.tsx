export const products = [
    ...
    content: (
      <div>
        ...
        <p>
          ...
          杜绝非法数据流入归档。所有竞价规则（报价校验、状态机、成交锁定）逐行移植自
          pm 桌面端，行为完全一致。
        </p>
      </div>
    ),
  ...
  },
  {
    ...
    content: (
      <div>
        ...
        <p>
          报表格式对齐 pm 桌面端：标的名称 / 序号 / 展示标题 / 起拍价 / 竞价幅度 /
  ...
  },
  {
    ...
    stack: ["Shunting-yard", "对齐 pm"],
    slug: "formula-engine",
    content: (
      <div>
        <p>
          公式引擎移植自 pm 的 Expression 求值器：公式可引用 DD（当前总价）与
          C1-C4（标的常量参数），支持四则运算与括号，采用 Shunting-yard
          算法做中缀转后缀求值，优先级与 pm 完全一致。
        </p>
        <p>
          默认公式对齐 pm 配置：GongSh4-7 = DD/C1、DD/C2、DD/(C1·C2)、空
          （空公式不计算）。变量为空 / 除零 / 语法错误时一律返回 0，
          与 pm 的 catch→0 行为一致，保证大屏展示永不因公式异常中断。
        </p>
      </div>
    ),
  },
